...
## 2024-05-27 - [Path Traversal in Plugin Uninstallation]
**Vulnerability:** A path traversal bypass was possible when canonicalization of a plugin directory path failed. The code used `.unwrap_or_else()` to fall back to the raw, uncanonicalized path. Since Rust's `Path::starts_with()` compares components rather than string prefixes, an attacker could use a path like `/plugins_base/../../../etc` which would bypass the check.
**Learning:** `std::fs::canonicalize()` can fail for reasons other than path traversal (e.g., if a file does not exist). Using uncanonicalized paths for security checks against base directories introduces vulnerabilities, as Rust's path resolution does not enforce physical location boundaries.
**Prevention:** Always handle path canonicalization errors explicitly (e.g., using `?` or `match`) and fail closed (reject the path) when canonicalization fails, rather than falling back to an uncanonicalized representation.
