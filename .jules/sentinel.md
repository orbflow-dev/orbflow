...
## 2024-09-02 - Path Traversal via Fail-Open Canonicalization
**Vulnerability:** Plugin uninstall allowed path traversal via `unwrap_or_else` on `std::fs::canonicalize` which fell back to an uncanonicalized path if the file did not exist.
**Learning:** When validating boundaries, canonicalization must fail-closed. Falling back to the raw string bypasses `starts_with` since `..` evaluates incorrectly.
**Prevention:** Always use `.map_err(...)` or `.unwrap()` instead of `.unwrap_or_else` on path canonicalizations when enforcing directory containment boundaries.
