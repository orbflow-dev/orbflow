...
## 2024-05-20 - [HIGH] Prevent path traversal bypass via fail-open canonicalization
**Vulnerability:** The application used `.unwrap_or_else()` to fall back to uncanonicalized paths when `std::fs::canonicalize()` failed. This bypassed the `.starts_with()` validation since `starts_with()` compares raw path components, meaning `/base/../target` evaluates as starting with `/base` if uncanonicalized.
**Learning:** Always fail closed on security-critical initialization steps like path canonicalization. A fallback to an insecure state completely negates the containment check.
**Prevention:** Use `.map_err()` to return an explicit error and deny access when `canonicalize()` fails instead of falling back to the uncanonicalized path.
