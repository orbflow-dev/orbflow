...
## 2024-09-12 - [Fail Securely on Canonicalization]
**Vulnerability:** Path traversal vulnerability due to using `.unwrap_or_else` on `canonicalize`, which bypassed the containment check (`starts_with`) when canonicalization failed (e.g. non-existent paths).
**Learning:** Using `.unwrap_or_else` to fallback to uncanonicalized paths allows bypasses of `starts_with` containment checks since `starts_with` works on path components.
**Prevention:** Always fail securely on canonicalization errors for system boundaries and do not fallback to raw strings.
