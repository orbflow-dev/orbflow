...
## 2024-09-10 - [CRITICAL] Prevent path traversal by strictly canonicalizing paths
**Vulnerability:** The application used `.unwrap_or_else()` to fall back to uncanonicalized paths when `std::fs::canonicalize` failed. This allowed attackers to bypass path traversal checks because `Path::starts_with` compares path components, meaning `/base/../target` evaluates as starting with `/base` if uncanonicalized.
**Learning:** `std::fs::canonicalize` failures must be treated as security failures (fail closed). Falling back to the raw path string negates the security guarantees of path canonicalization.
**Prevention:** Always fail closed on canonicalization errors. Do not use `.unwrap_or_else()` to bypass the error during security boundary checks.
