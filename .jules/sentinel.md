...
## 2024-07-22 - [Fail Securely on Canonicalization]
**Vulnerability:** Path Traversal bypass in plugin uninstall via `.unwrap_or_else` fallback.
**Learning:** When validating file boundaries using `starts_with()`, the validation is completely undermined if canonicalization fails open. Falling back to the raw, uncanonicalized string allows path traversal (`../`) components to persist and bypass the prefix check.
**Prevention:** File path security checks must fail securely (e.g., using `map_err`) when `canonicalize()` fails. Never fall back to uncanonicalized input for boundary validation.
