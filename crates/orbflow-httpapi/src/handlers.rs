...
pub async fn uninstall_plugin(
    ...
    let check_result = tokio::task::spawn_blocking(move || {
        ...
        // Canonicalize and verify it stays within plugins_dir (fail closed).
        let canonical_base = std::fs::canonicalize(&plugins_base).map_err(|_| "canonicalize")?;
        let canonical_dir = std::fs::canonicalize(&dir_check).map_err(|_| "canonicalize")?;
