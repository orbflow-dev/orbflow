...
impl CredentialProxy {
    ...
    pub fn new(cred_store: Arc<dyn CredentialStore>) -> Self {
        // Implement SSRF protection for CredentialProxy
        // Fails closed if the client fails to build (e.g. invalid TLS config) to prevent
        // silently falling back to a client lacking SSRF protections.
        let client = reqwest::Client::builder()
            .dns_resolver(Arc::new(ProxySsrfSafeResolver))
            .timeout(std::time::Duration::from_secs(30))
            .redirect(reqwest::redirect::Policy::none()) // proxy shouldn't follow redirects
            .build()
